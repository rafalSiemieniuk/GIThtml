...
(function ($) {
    ...
    if ($(window).width() > 1200) {
        ...
        $('.hdr__container').hide();
        $(window).scroll(() => {
            if (($('.section__about-us').offset().top - $(window).scrollTop()) < 10)
                $('.hdr__container').show(400);
        });
...
function animation(element) {
    ...
    $(window).scroll(() => {
        if ((positionTop - $(window).scrollTop()) <= $(window).height() * .8)
...
function windowScrollScope(element, scopeStart, scopeEnd, classCss) {
    $(window).scroll(() => {
        if ($(window).scrollTop() >= $(scopeStart).offset().top - 50 && $(window).scrollTop() <= $(scopeEnd).offset().top - 50)
